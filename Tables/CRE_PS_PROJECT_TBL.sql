/*$Id: CRE_PS_PROJECT_TBL.sql,v 1.7 2013/09/17 12:49:30 olando Exp $*/
...
/
alter table PS_PROJECT modify GLOBAL_NTE                VARCHAR2(255)
add
(
   -----------------------------------------------------
...
;
(
   GPMS_ID                   VARCHAR2(20) null,
   FILING_STRATEGY           VARCHAR2(500),
   LAUNCHING_SITE            VARCHAR2(500),
   PLFD_LATE                 DATE
)
/
-- Add comments to the columns 
comment on column PS_PROJECT.ACTION
  is 'I=INSERT U=UPDATE T=TERMINATE E= ERROR N=NONE';
comment on column PS_PROJECT.TIME_SENSITIVE
  is 'Y/N/Blank';
comment on column PS_PROJECT.LATE
  is 'Y/N/Blank';
comment on column PS_PROJECT.MARKET_ACTIVATED
  is 'Y/N/Blank';
comment on column PS_PROJECT.IS_GENERIC
  is 'Y/N/Blank';
-- Create/Recreate primary, unique and foreign key constraints 
-- 1. PK
alter table PS_PROJECT
  add constraint PS_PROJECT_PK primary key (PROJECT_ID)
  using index 
...
-- 2. UK
alter table PS_PROJECT
  add constraint PS_PROJECT_UK unique (PRJ_ID/*GPMS_ID*/, GPMS_EXP_ID, ENV_ACTIVITY_ID)
  using index 
  tablespace INTERFACES_DATA
  pctfree 10
  initrans 2
...
-- 3.1. FK 1
alter table PS_PROJECT
  add constraint PS_PROJECT_ENV_FK foreign key (ENV_ACTIVITY_ID)
  references PS_ENV_ACTIVITY (ENV_ACTIVITY_ID);
-- 3.2. FK 2
alter table PS_PROJECT
  add constraint PS_PROJECT_EXP_FK foreign key (GPMS_EXP_ID)
