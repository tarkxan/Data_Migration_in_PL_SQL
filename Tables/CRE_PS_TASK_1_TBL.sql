/*$Id: CRE_PS_TASK_1_TBL.sql,v 1.4 2013/05/02 15:36:22 olando Exp $*/
create table PS_TASK_1
(
   -----------------------------------------------------
   -- PREVIOUSLY EXISTED                              --
   -----------------------------------------------------
   ...
   --COUNTRY_ID                      VARCHAR2(50), -- ??? not in FDS, COUNTRY VARCHAR2(200),
   --
   --GPMS_ID                         VARCHAR2(20),
...
comment on column PS_TASK_1.ACTION
  is 'I=INSERT U=UPDATE T=TERMINATE E= ERROR N=NONE';
-- 1. PK 
alter table PS_TASK_1
  add constraint PS_TASK_DEV_PK_1 primary key (TASK_ID)
  using index 
  tablespace INTERFACES_DATA
  pctfree 10
  initrans 2
...
-- 2. UK
alter table PS_TASK_1
  add constraint PS_TASK_DEV_UK_1 unique (/*PROJECT_ID*/ PRJ_ID, /*COUNTRY_ID*/ COUNTRY, ENV_ACTIVITY_ID)
  using index 
  tablespace INTERFACES_DATA
  pctfree 10
  initrans 2
...
-- 3. FK
alter table PS_TASK_1
  add constraint PS_TASK_PROJECT_FK_1 foreign key(PROJECT_ID)
  references PS_PROJECT_1 (PROJECT_ID)
/
-- 4. Create/Recreate indexes 
create index PS_TASK_IND1 on PS_TASK_1 (/*GPMS_ID*/ PRJ_ID, /*COUNTRY_ID*/ COUNTRY, PROJECT_ID)
  tablespace INTERFACES_DATA
  pctfree 10
  initrans 2
  maxtrans 255
